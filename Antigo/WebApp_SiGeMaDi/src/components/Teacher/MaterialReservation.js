...
function Material_Reservation(props) {
    ...
    function onSubmit() {
        ...
        if (id != null) {
            props.history.push({
                pathname: `/auth/teacher/reservation/${id}/checkout`,
            })
        }
        else {
            props.history.push({
                pathname: '/auth/teacher/newreservation/checkout',
            })
        }
    }
    ...
    return (
        <div>
            ...
            {
                error ? error :
                materialTypes.length==0?<Message>Loading...<Icon name='circle notched' loading/></Message>:
                    <div>
