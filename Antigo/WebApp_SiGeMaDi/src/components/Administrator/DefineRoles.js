...
import { rolesUrl, userRolesUrl, userUrl, sci_areasUrl } from '../Links'
...
function Define_Roles(props) {
            ...
            .then(resp_sciareas => {
                axios.get(userRolesUrl.replace(':id', id))
                    .then(r => {
                        ...
                        if (r.data['roles'].find(r => r.name == 'student' || r.name == 'employee') != undefined) {
                            toast({
                                type: 'error',
                                time: 2000,
                                size: 'mini',
                                title: 'Invalid Parameter',
                                description: 'It is not permited change either student or employee role'
                            })
                            setTimeout(() => {
                                ...
                                .then(resp_roles => {
                                    ...
                                    axios.get(userUrl.replace(':id', id))
                                        .then(resp => {
                                            ...
                                            let availableRoles = resp_roles.data['roles'].filter(r => r != 'student')
    ...
    function onSubmit() {
        ...
        //mudar para ingles ver depois
        if (checked.find(r => r === 'lab_responsible')) {
            if (sci_area === 'all')
                return toast({
                    type: 'warning',
            ...
            const idx = checked.indexOf('lab_responsible')
            checked[idx] = `lab_responsible;${sci_area}`
        }
